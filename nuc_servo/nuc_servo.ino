...
#define SERVOC_1  1024 // This is the 'minimum' pulse length count (out of 4096)
#define SERVOAC_1  3072 // This is the 'maximum' pulse length count (out of 4096)
...
int gripper_ka_direction=0;
int gripper_ka_speed=0;
void setup() {
  Serial.begin(9600);
  ...
  delay(10); 
  pwm.setPWM(0, 0, SERVOSTOP_1);
  pwm.setPWM(1, 0, pulselen_1);// Start serial communication
...
//      pulselen=pulselen-5;
//    }
    int auto_command=main_command;
    // int auto_command=main_command/10;
    // int gripper_rot=main_command%10;
    // switch(main_command){
    //     case 22:
    ...
    gripper_ka_direction=main_command%10 ;

    if (gripper_ka_direction==2){
      gripper_ka_direction = -1 ;
    }
    else if(gripper_ka_direction==1){
      gripper_ka_direction=1;
    }
    else{
      gripper_ka_direction=0;
    }
    
    gripper_ka_speed=main_command/10;

    main_command = (gripper_ka_speed/120.0)*90*gripper_ka_direction ;

    pulselen_1=map(main_command,-90,90,SERVOMIN,SERVOMAX);
    
    pulselen_0=map(auto_command,-90,90,1024,3072);
